...
class UserManager(BaseUserManager):
    ...
    def create_user(self, username:str, email:str, password: Optional[str] = None) -> 'User':
        """CREATE AND RETURN A User wtih an email and password"""
        if username is None:
    ...
    def create_superuser(self, username:str, email:str, password:str) -> 'User':
        """Create and return a super user with admin permissions"""
        if password is None:
...
class User(AbstractBaseUser, PermissionsMixin):
    ...
    full_name = models.CharField(max_length=2000, null=True)
